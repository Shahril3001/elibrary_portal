	    ...
	    $member = new Member('','','','','','','','','');
			if(empty($memberEmail) || empty($memberPassword)) {
				...
				echo "</div>";
				 ...
				 $loginResult = $member->login($memberEmail, $memberPassword);

				if ($loginResult["success"]) {
					$loginNotification .= "alert('You Have Successfully Logged In.');\n";
					header("location:member-index.php?memberEmail=" . $loginResult["memberEmail"] . "&role=" . $loginResult["role"]);
				} else {
					...
					if($_SESSION['user-valid']<=2){
						// Display the error message for unsuccessful login attempts.
						echo "<div class='pos'>";
						echo "<img src='icon/icons8-error-96.png'/>";
